...
const blank={idParticipante:'',nombre:'',centro:'',area:'',temporada:new Date().getFullYear(),elegible:true,estatus:'Vigente',expedienteEstado:'Incompleto',expedienteUrl:'',observaciones:''};
...
const columnsPrint=[
 {label:'ID',key:'idParticipante'},{label:'Colaborador / trabajador',key:'nombre'},{label:'Centro',key:'centro'},
 {label:'Área',key:'area'},{label:'Temporada',key:'temporada'},{label:'Elegible',value:x=>x.elegible!==false?'Sí':'No'},
...
export default function PadronModule(){
 ...
 const [search,setSearch]=useState(''),[centro,setCentro]=useState('Todos'),[estatus,setEstatus]=useState('Todos'),[expediente,setExpediente]=useState('Todos'),[quality,setQuality]=useState('Todos');
 const [selected,setSelected]=useState(new Set()),[detail,setDetail]=useState(null),[audit,setAudit]=useState([]);
 const [bulkOpen,setBulkOpen]=useState(false),[bulk,setBulk]=useState({centro:'',area:'',temporada:'',elegible:'',estatus:'',expedienteEstado:''});
 const [importRows,setImportRows]=useState([]),[importFileName,setImportFileName]=useState(''),[importCentro,setImportCentro]=useState('');
 ...
 const qualityOf=x=>{
  ...
  if(!x.centro)flags.push('Sin centro');
 ...
 const filtered=useMemo(()=>items.filter(x=>{
  const q=norm(search);
  if(q&&!norm(`${x.idParticipante||''} ${x.nombre||''} ${x.centro||''} ${x.area||''} ${x.estatus||''} ${x.expedienteEstado||''}`).includes(q))return false;
  if(centro!=='Todos'&&String(x.centro||'Pendiente')!==centro)return false;
  ...
  return true;
 }),[items,search,centro,estatus,expediente,quality,duplicateNames,duplicateIds,suspectFamily]);
 ...
 async function save(){
  ...
  if(!id&&form.centro)id=await nextParticipantId(form.centro,temporada);
  ...
  try{
   ...
   reset();setMsg(id?'Participante guardado.':'Participante guardado sin ID; falta centro.');await load();
 ...
 async function bulkEdit(){
  ...
  const changes={};
  if(bulk.centro)changes.centro=bulk.centro;
  if(bulk.area)changes.area=bulk.area;
  ...
  setBulkOpen(false);setBulk({centro:'',area:'',temporada:'',elegible:'',estatus:'',expedienteEstado:''});setMsg(`${selectedRows.length} registro(s) actualizados.`);await load();
 }
 async function assignMissingIds(){
  const rows=selectedRows.filter(x=>!x.idParticipante&&x.centro);
  if(!rows.length){setMsg('Los seleccionados sin ID también deben tener centro para asignarlo.');return;}
  if(!confirm(`¿Asignar ID automáticamente a ${rows.length} participante(s) seleccionados?`))return;
  for(const x of rows){
   const id=await nextParticipantId(x.centro,Number(x.temporada)||new Date().getFullYear());
 ...
 async function mergeSelected(){
  ...
  const merged={...duplicate,...primary,
   ...
   centro:primary.centro||duplicate.centro||'',
 ...
 function exportRows(rows,name='padron_participantes_NODO.xlsx'){
  ...
  const data=rows.map(x=>({ID_PARTICIPANTE:x.idParticipante||'',COLABORADOR_TRABAJADOR:x.nombre||'',CENTRO:x.centro||'',AREA:x.area||'',TEMPORADA:x.temporada||'',ELEGIBLE:x.elegible!==false?'SI':'NO',ESTATUS:x.estatus||'',EXPEDIENTE_ESTADO:x.expedienteEstado||'',EXPEDIENTE_URL:x.expedienteUrl||'',OBSERVACIONES:x.observaciones||'',ACTIVIDADES:activityCounts.get(x.docId)||0,SOLICITUDES:requestCounts.get(x.docId)||0,OBSERVACIONES_CALIDAD:qualityOf(x).join(' | ')}));
 ...
 function downloadTemplate(){
  const ws=XLSX.utils.json_to_sheet([{COLABORADOR_TRABAJADOR:'',CENTRO:'',ID_PARTICIPANTE:'',AREA:'',TEMPORADA:new Date().getFullYear(),ELEGIBLE:'SI',ESTATUS:'Vigente',EXPEDIENTE_ESTADO:'Incompleto',EXPEDIENTE_URL:'',OBSERVACIONES:''}]);
 ...
 const centerCanon=v=>{const n=normKey(v);if(!n)return '';if(n.includes('bacum')||['01','1','bac'].includes(n))return 'Bácum';if(n.includes('caborca')||['02','2','cab'].includes(n))return 'Caborca';return '';};
 function validateImport(rows){
  ...
  return rows.map(r=>{
   ...
   if(!r.centro)warnings.push('Sin centro: no se asignará ID');
 ...
 function applyImportCenter(v){setImportCentro(v);setImportRows(validateImport(importRows.map(r=>r.centro?r:{...r,centro:v})));}
 ...
 return <div>
  ...
  <div id="alta-individual-padron" style={{...card,marginBottom:12}}>
   ...
   <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:9}}>
    <label style={{fontSize:12,fontWeight:800}}>ID participante<input style={input} value={form.idParticipante||''} placeholder="Automático al tener centro" onChange={e=>setForm({...form,idParticipante:e.target.value})}/></label>
    ...
    <label style={{fontSize:12,fontWeight:800}}>Centro<select style={input} value={form.centro} onChange={e=>setForm({...form,centro:e.target.value})}><option value="">Pendiente</option><option>Bácum</option><option>Caborca</option></select></label>
    <label style={{fontSize:12,fontWeight:800}}>Área<input style={input} value={form.area||''} onChange={e=>setForm({...form,area:e.target.value})}/></label>
  ...
  {showImport&&<div id="alta-plantilla-padron" style={{...card,marginBottom:12,border:`2px solid ${green}`}}>
   ...
   <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:8,marginTop:12}}>
    ...
    <div style={{padding:10,background:'#f6f8f3',borderRadius:9}}><b style={{color:green}}>2. Llenar en Excel</b><div style={{fontSize:11,color:muted,marginTop:4}}>El campo principal es <b>COLABORADOR_TRABAJADOR</b>. Puedes incluir centro, ID existente, área, temporada, elegibilidad, expediente y observaciones.</div></div>
    ...
    <div style={{padding:10,background:'#f6f8f3',borderRadius:9}}><b style={{color:green}}>4. Revisar e importar</b><div style={{fontSize:11,color:muted,margin:'4px 0 8px'}}>NODO muestra errores, actualizaciones y nuevos registros antes de confirmar.</div><select style={input} value={importCentro} onChange={e=>applyImportCenter(e.target.value)}><option value="">Centro por defecto (opcional)</option><option>Bácum</option><option>Caborca</option></select></div>
   </div>
   ...
   {importRows.length>0&&<div style={{overflowX:'auto',marginTop:8,border:`1px solid ${border}`,borderRadius:8,maxHeight:350}}>
    <table style={{width:'100%',borderCollapse:'collapse',fontSize:11,minWidth:850}}><thead style={{position:'sticky',top:0,background:'#f2f5ef'}}><tr>{['Fila','Colaborador / trabajador','Centro','ID','Estado de validación'].map(h=><th key={h} style={{textAlign:'left',padding:7,borderBottom:`1px solid ${border}`}}>{h}</th>)}</tr></thead><tbody>{importRows.map((r,i)=><tr key={i} style={{background:r._valid?'#fff':'#fff0f0'}}><td style={{padding:7,borderBottom:`1px solid ${border}`}}>{r._row}</td><td style={{padding:7,borderBottom:`1px solid ${border}`}}>{r.nombre||'—'}</td><td style={{padding:7,borderBottom:`1px solid ${border}`}}>{r.centro||'Pendiente'}</td><td style={{padding:7,borderBottom:`1px solid ${border}`}}>{r.idParticipante||r._existing?.idParticipante||'Automático'}</td><td style={{padding:7,borderBottom:`1px solid ${border}`,color:r._valid?(r._warnings?.length?warn:green):danger,fontWeight:700}}>{r._status}</td></tr>)}</tbody></table>
   </div>}
  ...
  <div style={{...card,marginBottom:12}}>
   <div style={{display:'grid',gridTemplateColumns:'minmax(240px,2fr) repeat(4,minmax(130px,1fr))',gap:7}}>
    <input style={input} value={search} placeholder="Buscar ID, nombre, centro, área..." onChange={e=>setSearch(e.target.value)}/>
    <select style={input} value={centro} onChange={e=>setCentro(e.target.value)}><option>Todos</option><option>Bácum</option><option>Caborca</option><option>Pendiente</option></select>
    ...
    <select style={input} value={quality} onChange={e=>setQuality(e.target.value)}><option>Todos</option><option>Con observaciones</option><option>Nombre duplicado</option><option>ID duplicado</option><option>Posible familiar/becado</option><option>Sin ID</option><option>Sin centro</option><option>Expediente pendiente</option></select>
   </div>
   ...
   {bulkOpen&&<div style={{marginTop:9,padding:10,background:'#f6f8f3',borderRadius:9}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:7,marginTop:7}}>
     <select style={input} value={bulk.centro} onChange={e=>setBulk({...bulk,centro:e.target.value})}><option value="">Centro: sin cambio</option><option>Bácum</option><option>Caborca</option></select>
     <input style={input} placeholder="Área: sin cambio" value={bulk.area} onChange={e=>setBulk({...bulk,area:e.target.value})}/>
  ...
  <div style={{overflowX:'auto',background:'#fff',border:`1px solid ${border}`,borderRadius:12}}>
   {busy?<div style={{padding:18}}>Cargando padrón…</div>:<table style={{width:'100%',borderCollapse:'collapse',fontSize:12,minWidth:1060}}>
    <thead style={{background:'#f4f7f1',position:'sticky',top:0,zIndex:1}}><tr>{['Sel.','ID','Colaborador / trabajador','Centro / área','Elegibilidad','Expediente','Historial','Calidad','Acciones'].map(h=><th key={h} style={{textAlign:'left',padding:'8px 7px',borderBottom:`1px solid ${border}`,color:green}}>{h}</th>)}</tr></thead>
    <tbody>{filtered.map(x=>{const q=qualityOf(x);return <tr key={x.docId} style={{background:suspectFamily.has(x.docId)?'#fff8f8':'#fff'}}>
     ...
     <td style={{padding:7,borderBottom:`1px solid ${border}`}}>{x.centro||'Pendiente'}{x.area&&<div style={{fontSize:10,color:muted}}>{x.area}</div>}</td>
  ...
  {detail&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.35)',zIndex:50,display:'flex',justifyContent:'flex-end'}} onClick={()=>setDetail(null)}>
   <div style={{width:'min(720px,96vw)',height:'100%',overflow:'auto',background:'#f7f8f5',padding:18,boxSizing:'border-box'}} onClick={e=>e.stopPropagation()}>
    <div style={{display:'flex',justifyContent:'space-between',gap:8}}><div><h2 style={{margin:'0 0 3px',color:green}}>{detail.nombre}</h2><div style={{fontSize:12,color:muted}}>ID {detail.idParticipante||'pendiente'} · {detail.centro||'Sin centro'} · {detail.estatus||'Vigente'}</div></div><button style={btn()} onClick={()=>setDetail(null)}>Cerrar</button></div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:8}}>
     <div style={card}><b>Datos del padrón</b><div style={{fontSize:12,marginTop:6}}>Centro: {detail.centro||'—'}<br/>Área: {detail.area||'—'}<br/>Temporada: {detail.temporada||'—'}<br/>Elegible: {detail.elegible!==false?'Sí':'No'}<br/>Expediente: {detail.expedienteEstado||'Incompleto'}</div></div>
