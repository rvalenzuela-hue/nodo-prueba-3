...
export default function ProgramasActividades(){
 ...
 function campoCentroParticipante(){
  return (actividad.campos||[]).find(c=>normaliza(c.etiqueta).includes('centro'))||null;
 }
 ...
 function recalcularFila(r){
  ...
  const nc=campoNombreParticipante(),cc=campoCentroParticipante();
  const nombre=String(datos[nc?.id]||'').trim();
  const hit=encontrarEnPadron(nombre);
  return {...r,datos,nombre,idParticipante:hit?.idParticipante||'',docId:hit?.docId||'',padronEstado:hit?'Registrado en padrón':'No registrado',centroAlta:String(datos[cc?.id]||hit?.centro||r.centroAlta||'').trim()};
 }
 function procesarBloquePegado(texto){
  ...
  const centroCampo=campoCentroParticipante();
  const parsed=matriz.filter(row=>row.some(v=>String(v||'').trim())).map((vals,i)=>{
    ...
    return {
      ...
      agregarPadron:false,
      centroAlta:String(datosConReglas[centroCampo?.id]||hit?.centro||'').trim(),
      origen:'Copia y pega'
    };
  });
 ...
 function pegarEnCelda(e,filaInicio,colInicio){
  ...
  setFilasPegadas(prev=>{
    ...
    while(rows.length<needed)rows.push({importId:`grid-${Date.now()}-${rows.length}-${Math.random().toString(36).slice(2,5)}`,nombre:'',datos:{},idParticipante:'',docId:'',padronEstado:'No registrado',agregarPadron:false,centroAlta:'',origen:'Copia y pega'});
 ...
 function agregarFilaVacia(){
  setFilasPegadas(prev=>[...prev,{importId:`grid-${Date.now()}-${Math.random().toString(36).slice(2,5)}`,nombre:'',datos:{},idParticipante:'',docId:'',padronEstado:'No registrado',agregarPadron:false,centroAlta:'',origen:'Copia y pega'}]);
 }
 ...
 async function agregarFilaPegadaAlPadron(row){
  ...
  try{
   ...
   if(!row.centroAlta)throw new Error(`Selecciona el centro para ${row.nombre}.`);
   const now=new Date().toISOString(),by=auth.currentUser?.email||'';
   const idParticipante=await nextParticipantId(row.centroAlta,new Date().getFullYear());
   const docId=idParticipante.replace(/[^a-zA-Z0-9_-]/g,'-');
   await setDoc(doc(db,'participantesPrima',docId),{
    idParticipante,nombre:row.nombre,centro:row.centroAlta,elegible:true,estatus:'Vigente',
   ...
   setPadron(prev=>[...prev,{docId,idParticipante,nombre:row.nombre,centro:row.centroAlta,elegible:true,estatus:'Vigente'}]);
 ...
 async function agregarSolicitudAlPadron(sol){
  ...
  try{
   ...
   const centroCampo=campoCentroParticipante();
   const centro=String(row.datos?.[centroCampo?.id]||'').trim();
   if(!centro)throw new Error(`La solicitud de ${row.nombre} no contiene centro. Agrega el campo Centro a los datos de participantes para poder asignar correctamente el ID.`);
   const now=new Date().toISOString(),by=auth.currentUser?.email||'';
   const idParticipante=await nextParticipantId(centro,new Date().getFullYear());
   const docId=idParticipante.replace(/[^a-zA-Z0-9_-]/g,'-');
   await setDoc(doc(db,'participantesPrima',docId),{
    idParticipante,nombre:row.nombre,centro,elegible:true,estatus:'Vigente',
  ...
  try{
    ...
    for(const row of filasPegadas){
      ...
      if(!row.idParticipante&&row.agregarPadron){
        ...
        if(!row.centroAlta)throw new Error(`Selecciona el centro para ${row.nombre} antes de agregarlo al padrón.`);
        const idParticipante=await nextParticipantId(row.centroAlta,new Date().getFullYear());
        const docId=idParticipante.replace(/[^a-zA-Z0-9_-]/g,'-');
        await setDoc(doc(db,'participantesPrima',docId),{
          idParticipante, nombre:row.nombre, centro:row.centroAlta,
