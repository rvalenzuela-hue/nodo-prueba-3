...
const makeRow=()=>({rid:`r-${Date.now()}-${Math.random().toString(36).slice(2,6)}`,nombre:'',centro:'',area:'',temporada:String(new Date().getFullYear()),elegible:'Sí',estatus:'Vigente',expedienteEstado:'Incompleto',observaciones:''});
...
export default function AltaCopiarPegarPadron({onDone}){
 ...
 async function save(){
  ...
  try{
   ...
   setMsg(`Proceso terminado: ${created} nuevos · ${updated} actualizados${skipped?` · ${skipped} omitidos por falta de centro`:''}.`);
 ...
 return <div style={{background:'#fff',border:`1px solid ${border}`,borderRadius:12,padding:14}}>
  ...
  <div style={{overflowX:'auto',border:`1px solid ${border}`,borderRadius:9}}><div style={{minWidth:1120}}>
   <div style={{display:'grid',gridTemplateColumns:'44px 240px 120px 150px 105px 95px 130px 130px 220px',background:'#edf2e9',fontSize:11,fontWeight:900}}><span style={{padding:8}}>#</span>{cols.map(c=><span key={c.key} style={{padding:8,borderLeft:`1px solid ${border}`}}>{c.label}</span>)}</div>
   {rows.map((r,ri)=><div key={r.rid} style={{display:'grid',gridTemplateColumns:'44px 240px 120px 150px 105px 95px 130px 130px 220px',borderTop:`1px solid ${border}`,fontSize:11}}><span style={{padding:8,textAlign:'center',background:'#f8f9f7'}}>{ri+1}</span>{cols.map((c,ci)=><input key={c.key} style={input} value={r[c.key]||''} onChange={e=>setCell(ri,c.key,e.target.value)} onPaste={e=>paste(e,ri,ci)}/>)}</div>)}
  </div></div>
